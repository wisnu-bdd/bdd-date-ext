export class DateX extends Date {
    ...
    getDeltaTimeCopyText(previous) {
        ...
        if (ds < 60)
            return "just now";
        if (ds >= 60 && ds < 60 * 60)
            return `${Math.floor(ds / 60)} minute${Math.floor(ds / 60) === 1 ? '' : 's'} ago`;
        if (ds >= 60 * 60 && ds < 24 * 60 * 60)
            return `${Math.floor(ds / (60 * 60))} hour${Math.floor(ds / (60 * 60)) === 1 ? '' : 's'} ago`;
